 ...
 *
 * Usage:
 *   npx ts-node scripts/generate-static-images.ts
 *
...
async function generateImage(prompt: string): Promise<string> {
  ...
  // For simplicity in this script, we'll poll the status
...
async function generateAllImages(): Promise<GenerationResult[]> {
...
function generateStaticImagesCode(results: GenerationResult[]): string {
  ...
  const formatEntry = (r: GenerationResult) => {
    if (r.url) {
      return `  '${r.id}': '${r.url}',`;
    }
...
async function main() {
  ...
  if (!NANOBANANA_API_KEY) {
    ...
    console.log('  npx ts-node scripts/generate-static-images.ts\n');
    process.exit(1);
  }
